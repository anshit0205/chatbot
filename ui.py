...
# --- HELPER: ROBUST GEOLOCATION (Swapped Order) ---
def get_current_location():
    """Tries the reliable API first, then falls back to geocoder"""
    
    # Priority 1: ipapi.co (Usually faster & more reliable)
    try:
        ...
        pass # If this fails, silently move to next method

    # Priority 2: Geocoder Library (Fallback)
    try:
        # We wrap this to catch the specific 'ipinfo' timeout error you saw
        g = geocoder.ip('me')
        if g.latlng:
            return g.latlng[0], g.latlng[1], g.city
    except Exception:
        pass # Silently fail if blocked
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            try:
                ...
                # INCREASED TIMEOUT: 10s -> 30s to prevent "Read timed out"
                ...
                reply = "⚠️ Connection Failed. Please make sure the backend is running (`uvicorn main:app`)."
