...
def format_time_ranges(slots):
    """
    Turns [6:00, 7:00, 8:00] into "06:00 AM - 09:00 AM"
    """
    ...
    for i in range(1, len(sorted_slots)):
        ...
        # Check if this slot is exactly 1 hour after the previous one
        if (current - last_slot_start).total_seconds() == 3600:
            last_slot_start = current # Extend range
        else:
            # Gap found! Close the previous range.
            # End time is start of last slot + 1 hour
...
async def chat_handler(req: ChatRequest):
    ...
    if intent == "get_address" or (target_name and "address" in req.message.lower()):
        # Double check: if target_name is 'near me', drop it
    ...
    if intent == "check_slots" or req_date or "slot" in req.message.lower():
        ...
        # SUB-CASE: Broad Search
            ...
            for c in centres:
                free_slots = get_available_slots(c['id'], req_date)
